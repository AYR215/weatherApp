...
function Local () {
    ...
    async function getWeather(lat, long) {
        if (lat&&long) {
            let response = await fetch(`https://api.openweathermap.org/data/2.5/onecall?lat=${lat}&lon=${long}&units=metric&alerts&appid=8fce7e4d96a4a5a6f7f597c1fff64f4e`)
    ...
    if (weather) {
        ...
        return (
                <div className="local_container">
                    ...
                    <p className="forecast">Wherever you are, there should be{cityName} { weather.current.weather[0].description }</p>
                    <p className="forecast">The temperature is { temp1.toFixed(1) } С and it feels like { temp2.toFixed(0) } С</p>
